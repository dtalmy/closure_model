...
def gr_Locationline(lat,long,PD_NC,tag,PD_out):
    ...
    if ds_time['bathy_mask']:
        ...
        ax0.plot(ps.dt.days, ds_time['c01'].values,'-g',label='First_phyto')
        ax0.plot(ps.dt.days, ds_time['c02'].values,'--m',label='Second_phyto')
        #ax0.set_xlim(0, 2)
        ax0.set_xlabel('time(days)')
        ax0.set_ylabel('plankton(mmol C/m^3)')
        ax0.grid(True)
        ax0.legend(bbox_to_anchor=(0,1.02,1,0.2), loc="lower left",
                    mode="expand", borderaxespad=0, ncol=3)
        #
        # ax2 = axs[0].twinx()
        # color = 'tab:red'
        ...
        world.plot(ax = ax2)
        str_title='Ext Loc: '+ str(gdf_loc.iloc[0].lats)+" "+str(gdf_loc.iloc[0].lons_pos) +" "+str(ds_time['bathy_mask'])
