...
def graph_zp_map(df_data,title,file_out):
    ...
    L_cruise=df_data.Name.unique()
    color_labels=df_data.Name.unique()
    rgb_values = sns.color_palette("rocket", color_labels.size )
    color_map = dict(zip(color_labels, rgb_values))
    for name_cruise in L_cruise:
        print(name_cruise)
        df_data.plot(ax=ax, marker='o', color=df_data['Name'].map(color_map), markersize=15,transform=ccrs.Geodetic(),label=str(name_cruise) )
    plt.axis('off')
    ax.legend()
...
def graph_zp_hist(df_data,title,file_out):
    ...
    for name_cruise in L_cruise:
        print(name_cruise)
        df_tmp=df_data.loc[df_data['Name'] == name_cruise]
        #data
...
def graph_zp_cruise(df_data,cruise,title,file_out):
    ...
    df_tmp=df_data.loc[df_data['Name'] == cruise]
...
def main():
    ...
    for name_cruise in L_cruise:
        ...
        graph_zp_cruise(df_data,name_cruise,title,file_out)
