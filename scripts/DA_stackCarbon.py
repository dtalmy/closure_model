...
#PD_NC=pathlib.Path('E:\\Projects\\OceanRelationship\\Random\\94053\\NC_trace')
# #PD_NC=pathlib.Path('E:\\Projects\\OceanRelationship\\Model_Process\\NP2Z1Dmay5\\NC_trace')
#PD_out=pathlib.Path('E:\\Projects\\OceanRelationship\\Random\\94053\\NC_analysis')
#tag='test_'
# NC_file=[\
#     'ptracers.c01.v4c.nc', \
#     'ptracers.c02.v4c.nc', \
...
def multi_stackCarbon(PD_NC,tag,PD_out):
    ...
    ax.set_title('Ind. trace global mean values at timestep')
    fig.tight_layout()
    ...
    plt.show()    
    #
    ds_tracers['tot']=ds_tracers['c01']+ds_tracers['c02']+ds_tracers['c03']+ds_tracers['c04']
   ...
   # gb_max=gb_zmean.groupby('tot').max()
    ...
    #suffix=str('max')
    #suffix=str('mean')
    print('compute xarray : start' , file = sys.stdout)
    ds_tracers=ds_tracers.compute()
    for suffix in L_suffix:
        s_suffix=str('_')+suffix
        print('Processing : ' +s_suffix, file = sys.stdout)
        ...
        #
        fig, ax = plt.subplots()
        ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['phyto'+s_suffix],c='green',label='Phyto')
        ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['zoo'+s_suffix]+ds_tracers['phyto'+s_suffix], c='red',label='Zoo')
        ax.set_title(suffix +' value over time and depth')
        ax.set_xlabel('Total of Planktonics C (mmol C/m^3)')
        ax.set_ylabel('Size fracionated C (mmol C/m^3)')
        ax.legend(loc=(1.04,0))
        ax2 = ax.twinx()
        color = 'tab:gray'
        ...
        fig.tight_layout()
        filename=tag+str('_')+suffix+str('_zpstack.png')
        file_out=PD_out.joinpath(filename)
        fig.savefig(file_out,dpi=300, bbox_inches='tight')
        #
        # individual tracer stack
        #
        fig, ax = plt.subplots()
        color = iter(plt.cm.Paired(np.linspace(0, 1,len(L_tracer) )))
        ds_tracers['incsum']=xr.zeros_like(ds_tracers['c01'+s_suffix])
        for trace in L_tracer:
            c = next(color)
            s_trace=str(trace)
            print('Plot trace : ' +s_trace, file = sys.stdout)
            ds_tracers['incsum']=ds_tracers['incsum']+ds_tracers[s_trace+s_suffix]
            ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['incsum'], c=c,label=s_trace)
        #ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['c01'+s_suffix]+ds_tracers['c02'+s_suffix],c='green',label='c02')
        #ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['c01'+s_suffix]+ds_tracers['c02'+s_suffix]+ds_tracers['c03'+s_suffix], c='red',label='c03')
        #ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['c01'+s_suffix]+ds_tracers['c02'+s_suffix]+ds_tracers['c03'+s_suffix]+ds_tracers['c03'+s_suffix], c='red',label='c04')
        ax.set_title(suffix +' value over time and depth')
        ax.set_xlabel('Total of ind. Planktonics C (mmol C/m^3)')
        ax.set_ylabel('Size fracionated C (mmol C/m^3)')
        ax.legend(loc=(1.04,0))
        ...
        ax2.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['zp'+s_suffix], alpha=0.5, facecolors='none', edgecolors=color,label='Z:P')
        fig.tight_layout()
        filename=tag+str('_')+suffix+str('_indstack.png')
        file_out=PD_out.joinpath(filename)
        fig.savefig(file_out,dpi=300, bbox_inches='tight')
        #plt.show() 
    # #
    ...
    # for suffix in L_suffix:
    #     s_suffix=str('_')+suffix
    #     ds_tracers['c_tot'+s_suffix]=ds_tracers['c01'+s_suffix]+ds_tracers['c02'+s_suffix]+ds_tracers['c03'+s_suffix]+ds_tracers['c04'+s_suffix]
    #     ds_tracers['phyto'+s_suffix]=ds_tracers['c01'+s_suffix]+ds_tracers['c02'+s_suffix]
    #     ds_tracers['zoo'+s_suffix]=ds_tracers['c03'+s_suffix]+ds_tracers['c04'+s_suffix]
    #     ds_tracers['zp'+s_suffix]=ds_tracers['zoo'+s_suffix]/ds_tracers['phyto'+s_suffix]
    #     #
    #     fig, ax = plt.subplots()
    #     ds_tracers['sum']=xr.zeros_like(ds_tracers['bathy_mask'])
    #     color = iter(plt.cm.Paired(np.linspace(0, 1,len(L_tracer) )))
    #     for trace in L_tracer:
    #         c = next(color)
    #         s_trace=str(trace)
    #         print('Plot Sum trace : ' +s_trace, file = sys.stdout)
    #         ds_tracers['sum']=ds_tracers['sum']+ds_tracers[s_trace+s_suffix]
    #         ax.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['c02'+s_suffix], c=c,label=s_trace)
    ...
    #     ax2.scatter(ds_tracers['c_tot'+s_suffix],ds_tracers['zp'+s_suffix], alpha=0.5, facecolors='none', edgecolors=color,label='Z:P')
    #     fig.tight_layout()
    #     filename=tag+str('_')+suffix+str('_stack_v2.png')
    #     file_out=PD_out.joinpath(filename)
    #     fig.savefig(file_out,dpi=300, bbox_inches='tight')
    #     #plt.show() 
    # #
